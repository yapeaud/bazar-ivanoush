            ...
            <header className='flex items-center justify-between py-5 font-medium'>
                ...
                <section className='flex items-center gap-6'>
                    ...
                    <article className='group relative'>
                        <img onClick={() => token ? null : navigate('/connexion')} src={assets.profile_icon} alt="" className='w-5 cursor-pointer' />
                        {/* Menu deéroulant */}
                        {token &&
                            <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4'>
                                <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-gray-500 rounded'>
                                    <p className='cursor-pointer hover:text-black'>Mon profil</p>
                                    <p onClick={() => navigate('/commandes')} className='cursor-pointer hover:text-black'>Commandes</p>
                                    <p onClick={logout} className='cursor-pointer hover:text-black'>Déconnexion</p>
                                </div>
                            </div>}
                    </article>
